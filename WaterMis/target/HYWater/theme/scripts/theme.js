//var SER_TYPE ; //ONLY, CHINA
function SerType() {
    jQuery.ajax({
        type : 'POST',
        contentType : 'application/json',
        url : 'config/getMapType' + "?"
        		+ Math.random(),
        dataType : 'json',
	...
	jQuery.ajax({
		...
		success : function(data) {
			...
			if (undefined == data) {
				...
				//$("#device_num").html(data.deviceSum);//与其后台再次查询 不如直接用区域的统计总数回显即可
				//$("#cityName").html("设备分布情况");
			}
		},
		error : function(data) {
			...
			.ajax({
				...
				success : function(data) {
					if (undefined == data || data.length < 1) {
						...
						$("#cityName").html('设备总数 : '+ deviceSum +" 套");//deviceSum
						var deviceHtml = "";
						console.log(data)
						for (var i = 0; i < data.length; i++) {
							values.name = data[i].name;
                        ...
                        $.post("/HYWater/config/getProjectPic", function(res) {
						    createMapOnly(maxNum, dataMsg, res.projectPic);
                        });
					}
				},
				error : function(data) {
...
function createMapOnly(maxNum,dataMsg,configRes){
	var dom = document.getElementById("map");
	var myChart = echarts.init(dom);
	option = null;
	// myChart.showLoading();

	//var mapJsonUrl = 'theme/scripts/map/'  + '320300_shouchuang.json';
	var mapJsonUrl = 'static/configResourse/'+configRes+'/map.json';
...
console.log(geoJson)
		...
		myChart.setOption({
			...
			dataRange: {
				...
				min: 0,
				max: maxNum,
				color:['#5bfd30', '#edff4a', '#51ffd1', '#F9F9F9'],
				text:['高','低'],
				textStyle: {
	            	color: '#fff'
	            },
...
function createMap(maxNum,dataMsg){
	...
	for(var i =0;i<dataMsg.length;i++){
		tempMap.put(dataMsg[i].name,dataMsg[i].value);
	}
	var dom = document.getElementById("map");
	var myChart = echarts.init(dom);
	var mydata = [  
	...
	for(var i =0;i<mydata.length;i++){
		if(null != tempMap.get(mydata[i].name) && tempMap.get(mydata[i].name) > 0){
			mydata[i].value = tempMap.get(mydata[i].name);
		}
	}
	var option = {
		    tooltip: {},
