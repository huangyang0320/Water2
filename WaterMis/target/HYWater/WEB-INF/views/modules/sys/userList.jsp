...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function page(n,s){
			if(n) $("#pageNo").val(n);
			if(s) $("#pageSize").val(s);
...
<body>
	...
	<form:form id="searchForm" modelAttribute="user" action="${ctx}/sys/user/list" method="post" class="breadcrumb form-search ">
		...
		<ul class="ul-form">
			<li><label>登录名：</label><form:input path="name" htmlEscape="false" maxlength="50" class="input-medium"/></li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="user">
			<tr>
				...
				<%-- <td><a href="${ctx}/sys/user/form?id=${user.id}">${user.hn}</a></td> --%>
				<td>${user.name}</td>
				<td>${user.sex}</td>
				<td>${user.phone}</td>
