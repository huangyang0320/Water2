function cascadeView(treeNode) {
	var forTag = "0";
	var authority = window.parent.GLOBAL.user["authority"][0];
	var city = [];
	for(var i=0;i<authority["children"].length;i++){
		for(var j=0;j<authority["children"][i]["children"].length;j++){
			if(authority["children"][i]["children"][j].id == treeNode.id){
				city = authority["children"][i]["children"][j];
				forTag = "1";
				break;
			}
		}
	...
	var deviceId = city["children"][0]["children"][0]["children"][0].id;
	window.parent.GLOBAL["deviceTree"].getNodesByFilter(function (node) {
        if (node.id === deviceId) {
        	window.parent.GLOBAL["deviceTree"].selectNode(node);
            return true;
        }
    }, true);
	modDefaultDB(treeNode.id);
	
}
var currentDevice = {
	id : window.parent.GLOBAL.cityTree.getSelectedNodes()[0].id,
	name : window.parent.GLOBAL.cityTree.getSelectedNodes()[0].name,
	dbName : window.parent.GLOBAL.cityTree.getSelectedNodes()[0]['treeInfo']['dbName']
};
...
function initPage(areaId) {
	jQuery.ajax({
		...
		url : parent.GLOBAL.contextPath + '/system/modDefaultDB/' + areaId
				+ "?" + Math.random(),
		dataType : 'json',
...
function summaryStat() {
	jQuery.ajax({
		type : 'POST',
		contentType : 'application/json',
		url : parent.GLOBAL.contextPath + '/count/summaryStat' + "?"
				+ Math.random(),
		dataType : 'json',
		success : function(data) {
			if (undefined == data) {
				...
				$("#device_pw").html(data.statPower == ''?'-':data.statPower);
				$("#device_num").html(data.deviceSum);
				$("#order_num").html(data.order_num == ''?'-':data.order_num);
...
function initStatisticHourAla(target, unit, name,charUrl) {
	jQuery.ajax({
		type : 'POST',
		contentType : 'application/json',
		url : parent.GLOBAL.contextPath + '/count/'+charUrl
				+ "?" + Math.random(),
		dataType : 'json',
		success : function(data) {
			if (data.length < 2) {
...
function countDeviceType() {
	jQuery.ajax({
		type : 'POST',
		contentType : 'application/json',
		url : parent.GLOBAL.contextPath + '/count/countDeviceType' + "?"
				+ Math.random(),
		dataType : 'json',
		success : function(data) {
...
function initWorkStat() {
	jQuery.ajax({
		type : 'POST',
		contentType : 'application/json',
		url : parent.GLOBAL.contextPath + '/count/workStat' + "?"
				+ Math.random(),
		dataType : 'json',
		success : function(data) {
			if (undefined == data || data.length < 1) {
				...
				var dataDQ = new Array();
				var dataYQ = new Array();
				var dataWC = new Array();
				for (var i = 0; i < data.length; i++) {
					var dateVal = new Object();
					dateVal.name = data[i].name;
					dateVal.value = parseInt(data[i].memo);
					if(dateVal.name.indexOf('完成') > -1){
						dataWC[dataWC.length] = dateVal;
					}
					if(dateVal.name.indexOf('待确认') > -1){
						dataDQ[dataDQ.length] = dateVal;
					}
					if(dateVal.name.indexOf('已确认') > -1){
						dataYQ[dataYQ.length] = dateVal;
					}
				}
				workStat(dataDQ,dataYQ,dataWC);
			}
		},
		error : function(data) {
...
function traSuppPumpStat() {
	jQuery.ajax({
		type : 'POST',
		contentType : 'application/json',
		url : parent.GLOBAL.contextPath + '/count/traSuppPumpStat' + "?"
				+ Math.random(),
		dataType : 'json',
		success : function(data) {
			if (undefined == data || data.length < 1) {
...
function countDeviceArea() {
	jQuery
			.ajax({
				type : 'POST',
				contentType : 'application/json',
				url : parent.GLOBAL.contextPath + '/count/countDeviceArea'
						+ "?" + Math.random(),
				dataType : 'json',
				success : function(data) {
					if (undefined == data || data.length < 1) {
...
function statisticRepaicType() {
	...
	jQuery.ajax({
		type : 'POST',
		contentType : 'application/json',
		url : parent.GLOBAL.contextPath + '/count/statisticRepaicType' + "?"
				+ Math.random(),
		dataType : 'json',
		success : function(data) {
			if (undefined == data) {
...
function staticUserWork() {
	jQuery.ajax({
		type : 'POST',
		contentType : 'application/json',
		url : parent.GLOBAL.contextPath + '/count/staticUserWork' + "?"
				+ Math.random(),
		dataType : 'json',
		success : function(data) {
...
function areaProjectRepairStat() {
	jQuery.ajax({
		type : 'POST',
		contentType : 'application/json',
		url : parent.GLOBAL.contextPath + '/count/areaProjectRepairStat' + "?"
				+ Math.random(),
		dataType : 'json',
		success : function(data) {
			if (undefined == data || data.length == 0) {
...
function initStatisticHourEle(time, type) {
	...
	jQuery.ajax({
		type : 'POST',
		contentType : 'application/json',
		url : parent.GLOBAL.contextPath + '/count/statisticHourEle/' + time
				+ "/" + type + "?" + Math.random(),
		dataType : 'json',
		success : function(data) {
			if (data.length < 2) {
...
function statisticAlarmType() {
	jQuery.ajax({
		type : 'POST',
		contentType : 'application/json',
		url : parent.GLOBAL.contextPath + '/count/statisticAlarmType'
				+ "?" + Math.random(),
		dataType : 'json',
		success : function(data) {
			if (undefined == data) {
...
function statisticPumpPressureInit(type) {
	...
	jQuery.ajax({
		type : 'POST',
		contentType : 'application/json',
		url : parent.GLOBAL.contextPath + '/count/statisticPumpPressure/'
				+ type + "?" + Math.random(),
		dataType : 'json',
		success : function(data) {
			if (undefined == data || data.length < 1) {
...
function initStatisticHourHot(dateTime, type) {
	...
	jQuery.ajax({
		type : 'POST',
		contentType : 'application/json',
		url : parent.GLOBAL.contextPath + '/count/hotChar/' + dateTime + '/'
				+ type+'/2' + "?" + Math.random(),
		dataType : 'json',
		success : function(data) {
...
function createMap(maxNum,dataMsg){
	...
	// myChart.showLoading();
	var mapJsonUrl = 'scripts/map/' + currentDevice.id + '.json';
...
function workStat(dataDQ,dataYQ,dataWC){
	// init pie : count order status
...
function countLocation() {
	jQuery.ajax({
		type : 'POST',
		contentType : 'application/json',
		url : parent.GLOBAL.contextPath + '/count/countLocation' + "?"
				+ Math.random(),
		dataType : 'json',
		success : function(data) {
			if (undefined == data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if (undefined != data[i].name) {
						dataVal.name = data[i].name;
						dataVal.value = parseInt(data[i].memo);
						sumVal += dataVal.value;
						dataMsg[i] = dataVal;
					}
				}
				localStat(dataMsg,sumVal);
			}
		},
		error : function(data) {
...
function localStat(dataMsg,sumVal){
	// 高低区设备数量统计
	var deviceNumCountChart = echarts.init(document.getElementById('deviceNumCount'));
	var deviceNumCountoption = {
		color: ['#eff71b', '#6da6ea', '#2c69b2', '#f25611'],
		title: {
	        text: sumVal,
	        subtext: '套',
	        x: 'center',
	...
	deviceNumCountChart.setOption(deviceNumCountoption);
}
//区位数据统计
function orderTypeStat() {
	$("#orderHtml").html("");
	jQuery.ajax({
		type : 'POST',
		contentType : 'application/json',
		url : parent.GLOBAL.contextPath + '/count/orderTypeStat' + "?"
				+ Math.random(),
		dataType : 'json',
		success : function(data) {
			if (undefined == data) {
				...
				var orderHtml = "";
				for (var i = 0; i < data.length; i++) {
					orderHtml += '<tr><td>'+(i+1)+'</td><td>'+data[i].name+'</td><td>'+data[i].memo+'</td></tr>';
					if('待确认' == data[i].name){
