    ...
    var settings = {}, roots = {}, caches = {},
        ...
        data = {
            ...
            getNodesByParamFuzzy: function (setting, nodes, key, value) {
                ...
                var reg= /^[A-Za-z]+$/;
                if (reg.test(value) && value.length<=4 ){ //判断是否符合正则表达式
                    var keyarr = value.split('');
                    let isPush = true;
                    for (var i = 0, l = nodes.length; i < l; i++) {
                        var node = nodes[i];
                        isPush = true;
                        if (typeof node[key] == "string" && $(nodes[i][key].toLowerCase()).find('p')) {
                            var pcontent = $(nodes[i][key].toLowerCase()).find('p').html();
                            if(pcontent){
                                keyarr.map(function (v, i, arr) {
                                    if(pcontent.indexOf(v)<=-1 || pcontent.indexOf(arr[i+1]) - pcontent.indexOf(v) > 4 ){
                                        isPush = false;
                                    }
                                })
                                if(isPush){
