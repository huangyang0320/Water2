...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        $("#showWarnDialog").click(function(){
          ...
          //   });
            // console.log(window)
            // document.childNodes[1].querySelector('#')
...
function leftmenu() {
    ...
    // 点击菜单条目加载对应页面
    $("#header-menu").find(".dropdown-menu").find("a").on("click",function(){
        closeVideo();
    ...
    $("#header-menu").find(".parent-menu").find("a").on("click",function(){
        closeVideo();
        var URL = $(this).attr("data-location");
        GLOBAL_URL_ADDRESS = URL;
        if(GLOBAL_URL_ADDRESS){
...
function toLoadingPage() {
    ...
    var cleanURL = ["systemOverview.html","themeStatNew.html","themeStatNew.html","energyRank.html", "pumpGuide.html", "pumpInfo.html", "video.html","videoHis.html", "alarmQuery.html", "doorAccess.html", "maintenance.html", "historyQuery.html","attendanceStat.html","accessRecord.html"];// 需要隐藏设备列表的页面
...
function getLeftMenuData() {
        ...
        $("#right-menu-list").load("rightmenu.html", function(){
           ...
           $("#hideMenu").click(function(){
               $(".right-menu").animate({top: '-500px'}, function(){
                    $(this).hide();
                    ...
                    if (result && result.length > 0) {
                        for (var i = 0; i < result.length; i++) {
                           ...
                           /* $li.find("a").append($("<span class='fa arrow'></span>"));*/
                            ...
                            if (menu["url"]) {
                                $li.find("a").attr("data-location", menu["url"] + ".html");
                                $li.find("a").attr("target", "mainBody");
                            ...
                            if (menu.hasOwnProperty("subMenu")) {
                                ...
                                if (subMenu && subMenu.length > 0) {
                                    //计算菜单换行
                                    var liLen=menu["liLen"];
                                    var liWidth=0;
                                    if(liLen==null || liLen==""){
                                        liWidth=(100*subMenu.length);
                                    }else{
                                        liWidth=(100*liLen);
                                    }
                                    if(liWidth>=500){
                        ...
                        $mainMenu.find("li").eq(0).find("li").eq(0).find("a");
...
function jumpMgSystem(){
    $.post(CONTEXT_PATH + "/sysConfig/getSysConfigByKey/MG_SYSTEM_URL", function (result) {
        //encrypt(GLOBAL_LOGIN_USER.loginName);
        //encrypt(GLOBAL_LOGIN_USER.encryPassword);
        window.open(result+"/login.html?username="+encrypt(GLOBAL_LOGIN_USER.loginName)+"&password="+encrypt(GLOBAL_LOGIN_USER.encryPassword),"location=no");
    });
}
...
function encrypt(word){
    var key = CryptoJS.enc.Utf8.parse("abcdefgabcdefgag");
...
function decrypt(word){
    ...
    var decryptStr = CryptoJS.enc.Utf8.stringify(decrypt).toString()
    console.log("解密："+decryptStr)
    return decryptStr;
}
...
function queryMaintenanceWorkerUser() {
    ...
    jQuery.ajax({
        ...
        success : function(data) {
            $("#executor").html("");
            $.each(data, function (i, item) {
                jQuery("#executor").append("<option value="+ item.id+">"+ item.name+"</option>");
            });
        }
    });
}
