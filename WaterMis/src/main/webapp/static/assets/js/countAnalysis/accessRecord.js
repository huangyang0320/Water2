...
function initBootTable(){
	...
	$('#dataTables-example').bootstrapTable({
		...
		detailFormatter : function(index, row) {// 详情信息
			...
			html.push('<p class="detail-view">' + '事件名称' + ' : '
					+ toTrim(row.eventName) + '</p>');
			html.push('<p class="detail-view">' + '卡号' + ' : '
					+ toTrim(row.eventCard) + '</p>');
			html.push('<p class="detail-view">' + '门禁名称'
					+ ' : ' + toTrim(row.doorName) + '</p>');
			html.push('<p class="detail-view">' + '设备名称'
					+ ' : ' + toTrim(row.deviceName) + '</p>');
			return html.join('');
		}
    });
    /* 饼图高度 */
    ...
    $("#countSubmit").ajaxSubmit( { 
 	    ...
 	    success : function(data){
 	    	...
 	    	for(var i=0;i<data.length;i++){
 	    		dataMsg[i] = new Array();
 	    		dataMsg[i][0] = data[i].eventName +"(" + data[i].eventType +" 天)";
